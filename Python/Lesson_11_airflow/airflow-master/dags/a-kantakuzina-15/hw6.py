...
# переменная NUMBER - это не переменная python (да, можно и password хранить в принципе). Зачем нужен аргумент env, можно тут прочитать:
#  https://airflow.apache.org/docs/apache-airflow/stable/_api/airflow/operators/bash/index.html
# "If env is not None, it must be a dict that defines the environment variables for the new process"
# То есть с помощью env мы определяем переменную окружения.
